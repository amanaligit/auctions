        ...
        if request.user.is_authenticated:
            if 'submit_comment' in request.POST:
                ...
                if form.is_valid():
                    ...
                    if bid > listing.bid:
                        Bid(username=request.user, bid=bid,
                            listing=listing).save()
                        update_listing(id=listing_id, price=bid,
                                       user=request.user)
                        return HttpResponseRedirect("/"+str(listing_id))
                    else:
...
def index(request):
    return render(request, "auctions/index.html", {'Listings': Listing.objects.filter(closed=False).order_by('-time')})
...
def create_listing(request):
    if request.method == 'POST':  # If the form has been submitted...
        ...
        if form.is_valid():
            l = Listing(username=request.user.username, title=form.cleaned_data['title'], bid=form.cleaned_data['price'],
                        desc=form.cleaned_data['desc'], image=form.cleaned_data['image'], time=datetime.datetime.now(), closed=False)
            l.save()
